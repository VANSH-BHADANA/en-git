...
export function ProgressReport({ report }) {
  ...
  if (changes?.followers >= 50) {
    achievements.push({
      icon: "🌟",
      text: `Gained ${changes.followers} followers! Rising Star!`,
      type: "major",
    });
  } else if (changes?.followers >= 25) {
    achievements.push({
      icon: "🚀",
      text: `Gained ${changes.followers} followers! Growing Fast!`,
      type: "medium",
    });
  } else if (changes?.followers >= 10) {
    achievements.push({
      icon: "👥",
      text: `Gained ${changes.followers} followers!`,
      type: "minor",
    });
  } else if (changes?.followers > 0) {
    achievements.push({
      icon: "✨",
      text: `+${changes.followers} new follower${changes.followers > 1 ? "s" : ""}`,
      type: "minor",
    });
  }
  ...
  if (changes?.repos >= 5) {
    achievements.push({
      icon: "🏗️",
      text: `Created ${changes.repos} new repositories! Productive!`,
      type: "medium",
    });
  } else if (changes?.repos > 0) {
    achievements.push({
      icon: "📦",
      text: `Created ${changes.repos} new repositor${changes.repos > 1 ? "ies" : "y"}`,
      type: "minor",
    });
  }
  ...
  if (changes?.forks >= 10) {
    achievements.push({
      icon: "🔱",
      text: `${changes.forks} new forks! Developers love your code!`,
      type: "medium",
    });
    ...
    achievements.push({
      icon: "🍴",
      text: `+${changes.forks} fork${changes.forks > 1 ? "s" : ""}`,
      type: "minor",
    });
  }
            ...
            <div className="space-y-2">
              {achievements.map((achievement, i) => (
                ...
                >
                  ...
                  <span
                    className={`text-sm ${achievement.type === "major" ? "font-semibold" : ""}`}
                  >
                    {achievement.text}
                  </span>
                  {achievement.type === "major" && (
